...
data Play = Fold | Check | Open [Chip] | Call [Chip] | Raise [Chip] | AllIn [Chip] deriving Show

data Status = Folded | Bet [Chip] | AllIned [Chip] deriving Show
...
determineCombination table player = go getters getHighCard Nothing (sortBy (comparing Down) $ table ++ cards player)
  where go :: [[Card] -> Maybe Combination] -> ([Card] -> Combination) -> Maybe Combination -> [Card] -> Combination
...
getStraightFlush (highest:rest)
  ...
  where go (Card s1 r1, Card s2 r2) = s1 == s2 && isDescByOne r1 r2
...
getFlush cards
  ...
  where go idx
        ...
        construct idx = Just $ Flush (head cds) (cds !! 1) (cds !! 2) (cds !! 3) (cds !! 4)
          where cds = filter (\c -> suit c == suit (cards !! idx)) cards
...
getStraight (highest:rest)
  ...
  where construct cards = Just $ Straight (head cards) (cards !! 1) (cards !! 2) (cards !! 3) (cards !! 4)
...
getThreeOfAKind cards
  ...
  where go idx
        ...
        construct idx = Just $ ThreeOfAKind (head three) (three !! 1) (three !! 2) (head other) (other !! 1)
          where three = filter (\c -> c == (cards !! idx)) cards
...
getPair cards
  ...
  where go idx
        ...
        construct idx = Just $ Pair (head pair) (pair !! 1) (head other) (other !! 1) (other !! 2)
          where pair = filter (\c -> c == (cards !! idx)) cards
                other = filter (\c -> c /= (cards !! idx)) cards
...
giveBank :: M.Map Int [Chip] -> Int -> [Player] -> [Player]
giveBank bank winner players = concat [take winner players, [winnerPlayer'], drop (winner + 1) players]
  where winnerPlayer = players !! winner
        winnerPlayer' = winnerPlayer {chips = mappend (chips winnerPlayer) $ concatMap snd (M.toList bank)}
